...
def transform_labels_sampling(bbox_labels, sample_bbox, resize_val,
    ...
    for i in range(len(bbox_labels)):
        ...
        if proj_bbox:
            ...
            if real_width * real_height < float(min_head_size * min_head_size):
                continue
            else:
                sample_label.append(bbox_labels[i][0])
                sample_label.append(float(proj_bbox.xmin))
...
def crop_image(img, bbox_labels, sample_bbox, image_width, image_height,
               resize_width, resize_height, min_head_size):
...
def preprocess(img, bbox_labels, mode, image_path):
        ...
        if prob > cfg.data_anchor_sampling_prob and cfg.anchor_sampling:
            ...
            if len(sampled_bbox) > 0:
                ...
                img, sampled_labels = crop_image_sampling(
                    ...
                    cfg.resize_width, cfg.resize_height, cfg.min_head_size)
            '''
    ...
    if mode == 'train':
        ...
        if mirror == 1:
            img = img[:, ::-1, :]
            for i in six.moves.range(len(sampled_labels)):
    ...
    return img/255.0, sampled_labels
